...
def edit_template(example: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Extract model's analysis for THINKING section
    model_analysis = example.get('full_response', '')
    
    # THINKING section (editable)
    st.write("**{THINKING} Section:**")
    corrected_thinking = st.text_area(
        "Model's detailed analysis (edit as needed)",
        value=model_analysis,
        height=300,
        key="edit_thinking",
        help="This should contain the step-by-step analysis from the model"
    )
    
    # RESPONSE section fields
    st.write("**{RESPONSE} Section:**")
    ...
    corrected_reasoning = st.text_area(
        "Reasoning (single clear sentence)",
        value=example.get('reasoning', ''),
        height=80,
        key="edit_reasoning"
    )
    
    # Generate formatted output preview
    formatted_output = f"""{{THINKING}}
{corrected_thinking}
/{{THINKING}}
...
/{{RESPONSE}}"""
    ...
    # Show preview of formatted output
    st.subheader("Formatted Output Preview")
    st.text_area(
        "This is how your gold standard will look:",
        value=formatted_output,
        height=200,
        key="preview_output",
        help="Copy this formatted output for your gold standard dataset"
    )
    
    # Copy button
    if st.button("📋 Copy Formatted Output to Clipboard", key="copy_button"):
        st.code(formatted_output, language="text")
        st.success("Formatted output displayed above - you can select and copy it!")
    
    return {
        'classification': corrected_classification,
        'victims': corrected_victims,
        'reasoning': corrected_reasoning,
        'thinking': corrected_thinking,
                    ...
                    if st.button("Save as Gold Standard", type="primary"):
                        ...
                        corrected_example = {
                            **current_example,  # Keep original data
                            ...
                            'correct_thinking': corrections['thinking'],
                            'formatted_gold_standard': corrections['formatted_output'],
            ...
            if st.session_state.corrected_examples:
                ...
                for ex in st.session_state.corrected_examples:
                    df_summary.append({
                        ...
                        'Has Formatted Output': 'Yes' if ex.get('formatted_gold_standard') else 'No',
